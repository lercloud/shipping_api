...
#         'address': fields.many2one('res.partner', 'Ship From Address'),
    ...
    def process_void(self, cr, uid, ids, context=None):
        ...
        return packages_pool.cancel_postage(cr, uid, [
            package.id for order in picking_pool.browse(cr, uid, ids) for package in order.packages_ids
        ], context=context)
...
#             logger.warning(_("Issue with opening the processed delivery order. %s." % e))
#             return ret
# 
#         return ret
# 
